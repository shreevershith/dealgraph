        ...
        <div className="space-y-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="animate-pulse">
              <div className="flex justify-between mb-1"><div className="h-3 bg-gray-700 rounded w-24" /><div className="h-3 bg-gray-700 rounded w-16" /></div>
...
function FounderCard({ name, status, result, claims }: { name: string; status: string; result?: string; claims?: Claim[] }) {
  ...
  } catch { resultText = result || ""; }

  const hasData = parsedData.length > 0;
  const isVerified = hasData && parsedData.some((d) => d.prev_company || d.current_company);
  const teamClaim = claims?.find((c) => c.category === "team" && c.text.toLowerCase().includes(name.split(" ")[0].toLowerCase()));
    ...
    <div className="rounded-xl border border-blue-500/20 bg-gradient-to-br from-gray-900 to-gray-950 p-5 my-3 w-full overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {status === "executing" ? (
              <span className="text-yellow-400 text-xs flex items-center gap-1.5"><span className="w-2 h-2 bg-yellow-400 rounded-full animate-pulse" />Verifying against graph...</span>
            ) : (
      ...
      {teamClaim && (
        <div className="mb-3 rounded-lg border border-purple-500/20 bg-purple-500/5 px-4 py-3">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${teamClaim.status === "verified" ? "bg-green-500/20 text-green-400" : teamClaim.status === "red_flag" ? "bg-red-500/20 text-red-400" : "bg-yellow-500/20 text-yellow-400"}`}>
              ...
              <div key={i} className="flex items-center gap-3 rounded-lg bg-gray-800/50 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {entry.current_company && !entry.prev_company && (
                    <p className="text-sm text-white font-medium">{entry.role || "Founder"} at <span className="text-purple-400">{entry.current_company}</span></p>
                  )}
                </div>
              </div>
            ))}
      ...
      {!hasData && resultText && status === "complete" && (
        <div className="mt-3 rounded-lg bg-gray-800/50 p-4">
          <p className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap">{resultText.length > 300 ? resultText.slice(0, 300) + "..." : resultText}</p>
        </div>
      )}
        ...
        <div className="space-y-2 mt-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex items-center gap-3 rounded-lg bg-gray-800/30 px-4 py-3 animate-pulse">
              <div className="w-2 h-2 rounded-full bg-gray-700" /><div className="h-3 bg-gray-700 rounded w-3/4" />
            </div>
          ))}
        </div>
      )}
...
function MarketCard({ marketName, status, result, claims }: { marketName: string; status: string; result?: string; claims?: Claim[] }) {
    ...
    <div className="rounded-xl border border-purple-500/20 bg-gradient-to-br from-gray-900 to-gray-950 p-5 my-3 w-full overflow-hidden">
          ...
          <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 p-3">
            ...
            {marketClaim ? (
              <>
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${marketClaim.status === "verified" ? "bg-green-500/20 text-green-400" : marketClaim.status === "red_flag" ? "bg-red-500/20 text-red-400" : "bg-yellow-500/20 text-yellow-400"}`}>
      ...
      {hasData && (
        <div className="grid grid-cols-2 gap-3">
          {tam && (<div className="rounded-lg bg-gray-800/50 p-3"><p className="text-gray-400 text-[10px] uppercase tracking-wider mb-1">Total Addressable Market</p><p className="text-white font-bold text-xl">{tam >= 1_000_000_000 ? `$${(tam / 1_000_000_000).toFixed(0)}B` : `$${(tam / 1_000_000).toFixed(0)}M`}</p></div>)}
          {growth && (<div className="rounded-lg bg-gray-800/50 p-3"><p className="text-gray-400 text-[10px] uppercase tracking-wider mb-1">Annual Growth Rate</p><p className="text-white font-bold text-xl">{growth}%</p><p className="text-green-400 text-[10px]">Year-over-year</p></div>)}
          {description && (<div className="rounded-lg bg-gray-800/50 p-3 col-span-2"><p className="text-gray-400 text-[10px] uppercase tracking-wider mb-1">Description</p><p className="text-gray-300 text-sm">{description}</p></div>)}
        </div>
      )}

      {!hasData && resultText && status === "complete" && (
        <div className="mt-3 rounded-lg bg-gray-800/50 p-4">
          <p className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap">{resultText.length > 400 ? resultText.slice(0, 400) + "..." : resultText}</p>
        </div>
      )}

      {status === "executing" && (
        <div className="grid grid-cols-2 gap-3 mt-3">
          {[1, 2, 3, 4].map((i) => (<div key={i} className="rounded-lg bg-gray-800/30 p-4 animate-pulse"><div className="h-2 bg-gray-700 rounded w-1/2 mb-2" /><div className="h-5 bg-gray-700 rounded w-3/4" /></div>))}
        </div>
      )}
    </div>
  );
...
function DealSummaryCard({ score, companyName, claimCount, competitorCount }: { score: DealScore; companyName: string; claimCount: number; competitorCount: number }) {
  const dimensions = [
  ...
  return (
    <div className="rounded-xl border border-amber-500/20 bg-gradient-to-br from-gray-900 to-gray-950 p-5 my-3 w-full overflow-hidden">
      ...
      <div className="mb-4 flex items-center gap-3">
        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold ${score.recommendation === "Strong Invest" ? "bg-green-500/20 text-green-400 border border-green-500/30" : score.recommendation === "Further Diligence" ? "bg-yellow-500/20 text-yellow-400 border border-yellow-500/30" : "bg-red-500/20 text-red-400 border border-red-500/30"}`}>
